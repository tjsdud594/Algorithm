...
로봇의 명령어는 문자열의 배열 instructions 로 주어진다. 
명령을 전부 수행 한 다음에 로봇의 방위각을 구하시오.
(방위각은 북쪽을 기존으로 로봇이 바라보는 각도이며, 방위각은 시계방향으로 측정이 된다.
 즉, 로봇이 서쪽을 바라보고 있다면 방위각은 270이 된다.)
...
리턴(정답): 0
왼쪽으로 90도 회전하면 로봇은 서쪽을 바라보게 된다 (방위각 270). 
다시 왼쪽으로 90도 회전하면 로봇은 남쪽을 바라보게 된다 (방위각 180). 
마지막으로 반 바퀴를 회전하면 로봇은 북쪽을 바라보게 된다 (방위각 0).
...
instructions = ["RIGHT 59","RIGHT","RIGHT","HALT","LEFT","LEFT","LEFT"]
리턴(정답): 239
...
def solution(self, instructions):
    ...
    for direction in instructions:
        ...
        if direction == "LEFT":
            ...
            answer += int(direction[6:])
        elif "LEFT " in direction:
            answer -= int(direction[5:])
        else:
            break
    return (answer%360)